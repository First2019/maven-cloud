 ...
 * 大费周章的构造各自本地的密钥对了。DH算法只能算非对称算法的底层实现。而RSA算法实现起来较为简单
 */
...
public class RSACoder {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("byte公钥12：\n" + publicKey[5]);
        String s = Base64.encodeBase64String(publicKey);
        byte[] bytes = Base64.decodeBase64(s);
        System.out.println("byte公钥12：\n" + bytes[5]);

        System.out.println("Base64公钥：\n" + Base64.encodeBase64String(publicKey));
        System.out.println("Base64私钥：\n" + Base64.encodeBase64String(privateKey));
